...
| `wf-publish-container.yml` | Check out a release tag and publish a Dockerfile-built image to GHCR. | Runner selectors, `environment-name`, image, context, Dockerfile, version, release tag, channel, and version build-arg name. | Optional `NODE_AUTH_TOKEN`. | `image-digest`, `image-reference`. | `contents: read`, `packages: write`. | `publish-ghcr` by default, override with `environment-name`. | Pushes immutable version tags and the channel aliases `dev[-latest]`, `rc[-latest]`, or `stable[-latest]`, plus `latest` for stable only. | `medrunner-services/ci/.github/workflows/wf-publish-container.yml@v1` |
...
The release job is the sole owner of semantic-release, Git tags, and stable-history backpropagation.
